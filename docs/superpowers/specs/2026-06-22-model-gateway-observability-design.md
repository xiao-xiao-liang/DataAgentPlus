...
实施时同时遵循项目 `AGENTS.md` 和《阿里巴巴 Java 开发手册》中适用于当前 Java、Spring Boot、MyBatis-Plus 技术栈的开发规范。

1. 代码注释和日志使用简体中文，每个类通过 JavaDoc 说明职责，关键方法按步骤添加中文注释。
2. 遵守阿里规范中的命名、常量、集合、并发、异常处理、日志、安全和数据库访问要求。
3. 新增线程池、有界队列、异步任务和资源许可时，必须显式定义容量、拒绝策略、超时、关闭与异常恢复行为，不使用无界资源。
4. 数据库表、索引和字段设计遵守阿里规范，并结合查询条件验证索引有效性；金额与成本计算使用明确精度和舍入规则。
5. 日志禁止字符串直接拼接，异常日志保留必要上下文，但不得输出密钥、Token、完整 Prompt、未脱敏响应或个人敏感数据。
6. 只修改当前阶段涉及的文件，不借机整理无关注释、日志、中文信息或历史代码风格。
7. 若通用规范与本项目明确约束冲突，以用户指令、`AGENTS.md` 和本设计文档为准，并在代码审查中说明原因。

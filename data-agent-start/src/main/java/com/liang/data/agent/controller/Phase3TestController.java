...
public class Phase3TestController {
    ...
    public Flux<String> testChat() {
        return llmService.callUser("你好，请做个详细的自我介绍。")
    ...
    public Flux<String> streamThreeNodes(@RequestBody StreamThreeNodesRequest request) {
        ...
        Flux<String> intentSource = llmService.callUser(PromptHelper.buildIntentRecognitionPrompt(multiTurn, query))
                .map(ChatResponseUtil::getText)
                .filter(StringUtils::hasLength);
    ...
    private Flux<String> buildEvidenceRecallFlux(StreamThreeNodesRequest request, String multiTurn, String query,
                                                 String intentOutput) {
        Flux<String> evidenceSource = llmService.callUser(PromptHelper.buildEvidenceQueryRewritePrompt(multiTurn, query))
                .map(ChatResponseUtil::getText)
                .filter(StringUtils::hasLength);
    ...
    private Flux<String> buildQueryEnhanceFlux(String multiTurn, String query, String evidence) {
        Flux<String> queryEnhanceSource = llmService.callUser(PromptHelper.buildQueryEnhancePrompt(multiTurn, query, evidence))
                .map(ChatResponseUtil::getText)
                .filter(StringUtils::hasLength);
